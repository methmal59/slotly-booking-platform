# Slotly â€” WhatsApp AI Booking Platform

A booking platform for salons and spas in Sri Lanka. Customers book, cancel, and reschedule appointments by chatting with an AI agent on WhatsApp â€” in Sinhala or English. Businesses get a dashboard, a public storefront, and automatic staff notifications.

It's running in production right now for Sri Lankan salons.

> **This repo is a case study, not the source code.** The platform is closed-source, so this page explains the architecture, the hard problems, and how I solved them â€” plus a few real code excerpts. Full commit history and a code walkthrough available on request.
...
<p align="center"><em>A full booking, end to end â€” every step tappable, typing works everywhere.</em></p>
...
Try something like *"I want to book a haircut tomorrow at 2pm"* â€” or type in Sinhala. The bot replies in Sinhala by default; say *"reply in English"* to switch. Every step comes with tappable buttons and lists, but you can also just type.
...
- **A WhatsApp AI agent** that handles the whole booking lifecycle in normal conversation â€” multiple services in one booking, staff preferences, availability, reschedules, cancellations. Every step is also tappable (WhatsApp lists and buttons).
...
| *"actually make it 3:30pm" â€” latest value wins, availability re-checked, summary re-shown* | *A slot that can't fit is explained honestly â€” never "someone just took it"* | *Casual spoken Sinhala, structured data stays English* |

*(Scripted demos of the production bot's real reply formats â€” message the demo number above to try it live.)*
...
```
                    ...
                    â”‚            Core API                â”‚
                    â”‚   FastAPI Â· async SQLAlchemy       â”‚
                    â”‚   MySQL (schema per tenant)        â”‚
                    â””â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
        platform JWT    â”‚          â”‚ tenant   â”‚ shared-secret
                        â”‚          â”‚ JWT      â”‚ service auth
              â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”      â”‚     â”Œâ”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
              â”‚  Platform   â”‚      â”‚     â”‚   AI Service      â”‚
              â”‚  Admin SPA  â”‚      â”‚     â”‚ FastAPI+LangGraph â”‚
              â”‚  (Next.js)  â”‚      â”‚     â”‚ + Gemini + Redis  â”‚
              â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜      â”‚     â””â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                            â”Œâ”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”   â”‚  WhatsApp Cloud API
                            â”‚ Tenant Web  â”‚   â”‚  (Meta) â‡„ customers
                            â”‚  (Next.js)  â”‚   â”‚
                            â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜   â””â”€ staff notifications
```

Four services, four repos, four independent deploy pipelines (GitHub Actions â†’ VPS/PM2, Render, Vercel). The core API is the hub. The AI service never touches a database â€” everything goes through the API over HTTP.

## Design decisions I'd defend in an interview

**One MySQL schema per business.** Not row-level `tenant_id` filtering â€” each business gets its own schema with its own encrypted credentials. A query bug in one tenant's context physically can't read another tenant's data. The cost is managing N schemas and a migration tool that applies changes across all of them. Worth it when you're holding other businesses' customer data.
...
**The bot follows the business's rules.** Salons that require approval get "booking requested" instead of instant confirmation. Booking-notice limits are enforced. Closed days never show up in date pickers. If a salon turns the bot off, it goes completely silent â€” but an API outage never mutes a live salon (everything fails open).

## Bugs that only production teaches you

All of these came from real customer conversations, not test plans:

- **"Only the haircut" didn't work.** A customer removing a service from their booking got a friendly "sure, removed!" â€” while the cart silently kept everything. The reply LLM was confirming things the code never did. Fix: cart changes are owned by deterministic code, and the reply is generated from the *actual* resulting cart, so the bot can't claim something that didn't happen.
- **A reschedule collided with itself.** Changing only the staff member made the appointment fail its own availability check â€” "that time is taken" â€” taken by the customer's own booking. Fix: reschedule availability checks now exclude the appointment being moved.
- **"6 of us at 9am" booked one person.** There's no group capacity model, so the headcount was silently dumped into a notes field. Now the NLU extracts party size (with a deterministic backstop scanning for headcounts it missed), and the bot says plainly that it can't book groups â€” and alerts a manager instead.
...
Every fix shipped with regression tests. The suite is at 1,701+ tests now.
...
- Per-customer rate limits (sliding window + daily cap) that fail open â€” for a booking bot, availability beats strictness.

## Sinhala that actually sounds right

This was more work than it sounds:

- The bot speaks **casual spoken Sinhala** â€” the register people actually text in â€” not the formal written form. Word choice matters: *staff*, not à·ƒà·šà·€à¶šà¶ºà· (which reads as "servant"). The voice was reviewed with a native-speaking product owner.
...
- Dates, times, and prices stay in English inside Sinhala sentences â€” matching how Sri Lankans text.
...
1. **1,701+ deterministic tests** with the LLM faked â€” units, full conversation-flow simulations, and five hardening tiers (silent failures, messy multi-intent conversations, weird inputs, odd configs, failure injection).
2. **A real-LLM eval suite** that runs scored conversation journeys against live Gemini â€” NLU accuracy, Sinhala reply quality, 33 adversarial attack prompts, edge-case recovery. Scores against thresholds, never exact text matching. Runs nightly in CI with a regression gate.
...
- **I built the API's AI-facing layer** â€” the 31 internal endpoints the bot uses, staff notifications, reminder scheduling â€” and contributed across the core API.
...
- [`excerpts/idempotency_guard.py`](excerpts/idempotency_guard.py) â€” the double-tap booking lock
- [`excerpts/note_sanitizer.py`](excerpts/note_sanitizer.py) â€” sanitizing customer free text before it travels
- [`excerpts/tenant_routing.py`](excerpts/tenant_routing.py) â€” routing webhooks to the right salon, with fail-open config caching

## Deep dives

Four longer reads for anyone who wants the full engineering story:

* [`ARCHITECTURE.md`](ARCHITECTURE.md) â€” the four services, the contracts between them, and the life of a message from webhook to reply
* [`EVALS.md`](EVALS.md) â€” how you test an LLM system: 1,701+ deterministic tests plus a nightly real-model eval suite with a regression gate
* [`RELIABILITY.md`](RELIABILITY.md) â€” production war stories: the day the model leaked its own prompt, timeouts that aren't failures, and choosing fail-open vs fail-safe
* [`COST_AND_LATENCY.md`](COST_AND_LATENCY.md) â€” sub-second webhook acks, a two-call-per-turn token ceiling, and measuring cost instead of guessing
...
*Udana Methmal Â· [github.com/methmal59](https://github.com/methmal59) Â· udana.methmal59@gmail.com*
